...
const bit<32> latency0 = 0;   // latency  - 10000000 - 10ms
const bit<32> latency1 = 0;   // latency  - 10000000 - 10ms
const bit<32> latency2 = 0;   // latency  - 10000000 - 10ms
const bit<32> latency3 = 0;   // latency  - 10000000 - 10ms
const bit<32> latency4 = 0;   // latency  - 10000000 - 10ms
const bit<32> latency5 = 0;   // latency  - 10000000 - 10ms
const bit<32> latency6 = 0;   // latency  - 10000000 - 10ms
const bit<32> latency7 = 0;   // latency  - 10000000 - 10ms
const bit<32> latency8 = 0;   // latency  - 10000000 - 10ms
...
control SwitchIngress(
    ...
    // Slice
    action slice_select_dst(bit<160> routeIdPacket){
        hdr.rec.routeid = routeIdPacket;
    }
        ...
        if (hdr.vlan_tag.isValid() && !hdr.rec.isValid() && !hdr.arp.isValid()) {
            ...
            if (hdr.rec.isValid()) {
		     ...
		     if(hdr.rec.signal==0){
                ...
                else if (hdr.rec.sw == 8){                   // 0 - ID switch
                    bit<8> value_tscal;
                    md.ts_diff = 0;
                    comp_diff();
    		     //apply the jitter
