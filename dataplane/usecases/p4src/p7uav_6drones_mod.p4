...
control SwitchIngress(
    ...
    bit<16> collision_4 = 0;
    bit<16> collision_4_l = 0;

    bit<16> collision_5 = 0;
    bit<16> collision_5_l = 0;

    bit<16> collision_6 = 0;
    bit<16> collision_6_l = 0;
    ...
    RegisterAction<bit<16>, bit<16>, bit<16>>(uav4_l) uav4_l_check_action = {
        ...
        RegisterAction<bit<16>, bit<16>, bit<16>>(uav5) uav5_update_action = {
        void apply(inout bit<16> value){
            value = md.x_pos_hex;
        }
    };

    RegisterAction<bit<16>, bit<16>, bit<16>>(uav5) uav5_check_action = {
        void apply(inout bit<16> value, out bit<16> readvalue){
            if (md.x_pos_hex > (value-10) && md.x_pos_hex < (value+10)){
                    readvalue = 1;
            }
    ...
    RegisterAction<bit<16>, bit<16>, bit<16>>(uav5_l) uav5_l_update_action = {
        void apply(inout bit<16> value){
            value = md.y_pos_hex;
        }
    };

    RegisterAction<bit<16>, bit<16>, bit<16>>(uav5_l) uav5_l_check_action = {
        void apply(inout bit<16> value, out bit<16> readvalue){
            if (md.y_pos_hex > (value-10) && md.y_pos_hex < (value+10)){
                readvalue = 1;
            }
        ...
        RegisterAction<bit<16>, bit<16>, bit<16>>(uav6) uav6_update_action = {
        void apply(inout bit<16> value){
            value = md.x_pos_hex;
        }
    };

    RegisterAction<bit<16>, bit<16>, bit<16>>(uav6) uav6_check_action = {
        void apply(inout bit<16> value, out bit<16> readvalue){
            if (md.x_pos_hex > (value-10) && md.x_pos_hex < (value+10)){
                    readvalue = 1;
            }
    ...
    RegisterAction<bit<16>, bit<16>, bit<16>>(uav6_l) uav6_l_update_action = {
        void apply(inout bit<16> value){
            value = md.y_pos_hex;
        }
    };

    RegisterAction<bit<16>, bit<16>, bit<16>>(uav6_l) uav6_l_check_action = {
        void apply(inout bit<16> value, out bit<16> readvalue){
            if (md.y_pos_hex > (value-10) && md.y_pos_hex < (value+10)){
                readvalue = 1;
            }
    ...
    apply { 
        ...
        if(!hdr.position.isValid() && hdr.tcp.dport == 5555){     
            ...
            if(hdr.id.id == set_position_request && md.collision_ok == 1){
                drop();
            }  
            ...
            if (hdr.position.object_id == 0x3831){
                uav4_update_action.execute(0);
                uav4_l_update_action.execute(0);
            } 
            else{
                collision_4 = uav4_check_action.execute(0);
                collision_4_l = uav4_l_check_action.execute(0);
            }

            if (hdr.position.object_id == 0x3832){
                uav1_update_action.execute(0);
                uav1_l_update_action.execute(0);
            } 
            else{
                collision_5 = uav5_check_action.execute(0);
                collision_5_l = uav5_l_check_action.execute(0);
            }

            if (hdr.position.object_id == 0x3833){
                uav6_update_action.execute(0);
                uav6_l_update_action.execute(0);
            } 
            else{
                collision_6 = uav6_check_action.execute(0);
                collision_6_l = uav6_l_check_action.execute(0);
            }
            ...
            if ((collision_1_l == 1) || (collision_2_l == 1) || (collision_3_l == 1) || (collision_4_l == 1) || (collision_5_l == 1) || (collision_6_l == 1) || (collision_1 == 1) || (collision_2 == 1) || (collision_3 == 1) || (collision_4 == 1) || (collision_5 == 1) || (collision_6 == 1)){
