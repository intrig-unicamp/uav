...
control SwitchIngress(
    ...
    Register <bit<32>, _> (32w2048)  tcp_seq_num;
    // Register to save ipv4 
    Register <bit<16>, _> (32w2048)  ipv4_len_num;
    // Register to save tcp len
    Register <bit<16>, _> (32w2048)  tcp_len_num;
    ...
    action collision_action(bit<16> new_object_id,
                            bit<48> new_x,
                            bit<48> new_y,
                            bit<48> new_z,
    ...
    apply { 
        ...
        if(!hdr.position.isValid() && hdr.tcp.dport == 5555){
            ...
            tcp_seq_num_action.execute(1);
            // Save the ipv4 id
            ipv4_len_num_action.execute(1);
            // Save the tcp segment len
            tcp_len_num_action.execute(1);
        }
            ...
            md.tcp_seq = tcp_seq_num_get_action.execute(1);
            md.ipv4_id = ipv4_len_num_get_action.execute(1);
            ipv4_tcp_len_reg = ipv4_len_num_get_action.execute(1);
            md.tcp_len = 0;
